...
class Data:
    ...
    def load():
        try:
            ...
            Data().placements = pd.read_csv(Data.__files['placements'], sep=';', header=[0, 1], index_col=0)
            print('Data loaded.')
    ...
    def add(game: Game):  # error due to circular import, fixed by __future__. Does not block correct script execution.
        return Data().__add_shots(game.datas).__add_placements(game.datas)
    ...
    def __add_placements(self, data: dict[str, pd.DataFrame]):
        ...
        placements.columns = pd.MultiIndex.from_product([[game_index], placements.columns], names=self.placements.columns.names)
